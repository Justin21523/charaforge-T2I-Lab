...
const GenerationPanel = () => {
    ...
    try {
      ...
      if (controlnetParams.enabled) {
        ...
        while (activeJobRef.current === jobId) {
          let status;
          try {
            status = await apiService.getT2IJobStatus(jobId);
          } catch (e) {
            toast.error(e?.message || "查詢狀態失敗", { id: "t2i-job" });
            break;
          }
      ...
      toast.error(error?.message || "生成失敗", { id: "t2i-job" });
    } finally {
      if (jobId) {
        if (activeJobRef.current === jobId) {
  ...
  }, [activeJobId, params, controlnetParams, apiCall, updateParam]);
  ...
  const handleDownload = useCallback(async (image) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        let message = `下載失敗 (HTTP ${response.status})`;
        try {
